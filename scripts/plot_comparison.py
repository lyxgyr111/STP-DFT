...
def plot_comparison(log1_path, label1, log2_path, label2):
    ...
    ax_main.plot(sorted_iters2, sorted_losses2, 's--', label=f'{label2} (验证损失)', color='royalblue', markersize=6,
                 linewidth=2)
    ax_main.set_title('模型性能对比：DFT vs. 标准Transformer', fontsize=20, fontweight='bold')
    ...
    ax_diff.text(0.5, 0.9, '差值 > 0: DFT模型更优', transform=ax_diff.transAxes,
                 ha='center', va='top', fontsize=10, color='blue')
    ax_diff.text(0.5, 0.1, '差值 < 0: 标准模型更优', transform=ax_diff.transAxes,
...
if __name__ == '__main__':
    ...
    parser.add_argument('dft_log', help='DFT模型的日志文件路径 (例如: dft_baseline_log.txt)')
    parser.add_argument('standard_log', help='标准模型的日志文件路径 (例如: standard_baseline_log.txt)')
    args = parser.parse_args()
    plot_comparison(args.dft_log, 'DFT 模型', args.standard_log, '标准模型')